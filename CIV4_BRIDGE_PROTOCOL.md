...
can_build {"x":10,"y":12,"build":4,"player":0,"test_visible":false}
```
...
production cost. Reply with `{"value":1}` to `can_build` to force allow, `{"value":0}` to veto,
or a negative value to continue normal DLL checks.
...
- `BridgeEvent` typed variants for mirrored `CvEventReporter` payloads, city production rule callback requests, `get_building_cost_mod`, `unit_cannot_move_into`, player research callbacks, civic callbacks, and `can_build`, plus `BridgeEventKind`, `CityProductionRule`, and `CityProductionItem`
